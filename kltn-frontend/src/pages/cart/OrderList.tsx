  ...
  const handleCancelOrder = async (orderId: number) => {
    const result = await Swal.fire({
      ...
      customClass: {
        popup: "rounded-2xl",
        confirmButton:
          "px-6 py-2.5 rounded-lg font-medium shadow-lg hover:shadow-xl transition",
        cancelButton:
          "px-6 py-2.5 rounded-lg font-medium shadow-md hover:shadow-lg transition",
      },
      buttonsStyling: true,
      reverseButtons: true,
      focusCancel: true,
        ...
        html: `
                    <div class="text-center">
                        ...
                        <p class="text-gray-600 mb-2">${
                          error.message || "Đã xảy ra lỗi khi hủy đơn hàng"
                        }</p>
    ...
    return (
      <div className="flex justify-center items-center space-x-3 mt-8">
        ...
        {pages.map((page) => (
          <button
            key={page}
            className={`w-10 h-10 rounded-full font-medium transition duration-200 ${
              currentPage === page
        ...
        <button
          className={`px-4 py-2 rounded-lg flex items-center transition duration-200 ${
            currentPage === orderPage.totalPages - 1
              ? "bg-gray-100 text-gray-400 cursor-not-allowed"
              : "bg-red-50 text-red-500 hover:bg-red-100"
          }`}
          onClick={() => handlePageChange(currentPage + 1)}
    ...
    <div className="container mx-auto p-4 lg:p-6 lg:max-w-6xl">
      <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-2xl p-6 mb-8 flex items-center">
        ...
        <div className="hidden md:block">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-24 w-24 text-red-400 opacity-75"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      {orderPage.content.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
          ...
          <Link
            to="/"
            className="inline-flex items-center bg-red-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-red-600 transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            {orderPage.content.map((order: any) => (
              ...
              >
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 pb-4 border-b border-gray-100">
                  ...
                  <div
                    className={`px-4 py-2 rounded-lg text-sm border flex items-center ${getStatusColor(
                      order.status
                    )}`}
                  >
                ...
                <div className="space-y-4 mb-6">
                  <h4 className="font-semibold text-gray-800 flex items-center">
                    ...
                    Sản phẩm đã đặt ({order.order_details?.length || 0} sản
                    phẩm)
                  </h4>
                  {order.order_details &&
                    order.order_details.map((item: any, index: number) => (
                      <div
