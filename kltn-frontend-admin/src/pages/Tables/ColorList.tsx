        ...
        <div>
            ...
            <div className="mb-6 rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark">
                <div className="flex justify-between items-center">
                    ...
                    <div>
                        <button onClick={openCreateModal} className="rounded bg-primary px-4 py-2 text-white">Create Color</button>
                    </div>
                </div>
                ...
                <div className="mt-4">
                    <table className="w-full table-auto">
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr><td colSpan={4} className="text-center py-6">No colors found.</td></tr>
                            ) : colors.map(c => (
                                <tr key={c.id}>
                    ...
                    <div className="flex justify-center mt-4">
                        {Array.from({ length: totalPages }, (_, i) => (
                            <button key={i} onClick={() => fetchColors(i)} className={`mx-1 px-3 py-1 rounded ${page === i ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-meta-4'}`}>
                                {i + 1}
                            </button>
                        ))}
                    </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                    <div className="w-full max-w-lg rounded bg-white p-6 dark:bg-boxdark">
                        <h3 className="font-medium mb-4">{editingColor ? 'Edit Color' : 'Create Color'}</h3>
