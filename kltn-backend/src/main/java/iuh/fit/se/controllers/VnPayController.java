...
public class VnPayController {
  ...
  public APIResponse<?> returnPayment(@RequestParam("vnp_ResponseCode") String responseCode) {
    try {
      ...
      return APIResponse.builder().message("Payment handled successfully").result(result).build();
    } catch (Exception e) {
      return APIResponse.builder().message("Error handling payment: " + e.getMessage()).build();
    }
  }
}
