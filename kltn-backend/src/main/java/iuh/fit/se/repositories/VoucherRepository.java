...
public interface VoucherRepository extends JpaRepository<Voucher, Long> {
  ...
  // Find vouchers that user can claim (not already claimed, has usage limit, and active)
  @Query(
      "SELECT v FROM Voucher v "
          + "WHERE v.active = true "
          ...
          + "AND (v.startDate IS NULL OR v.startDate <= CURRENT_TIMESTAMP) "
          + "AND (v.endDate IS NULL OR v.endDate >= CURRENT_TIMESTAMP) "
          + "AND v.usageLimit > (SELECT COALESCE(SUM(uv.usageCount), 0) FROM UserVoucher uv WHERE uv.voucher.id = v.id) "
          + "AND NOT EXISTS (SELECT 1 FROM UserVoucher uv2 WHERE uv2.voucher.id = v.id AND uv2.user.id = :userId) "
          + "AND (:keyword IS NULL OR :keyword = '' OR LOWER(v.code) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(v.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
  Page<Voucher> findClaimableVouchersForUser(
      @Param("userId") Long userId, @Param("keyword") String keyword, Pageable pageable);
}
