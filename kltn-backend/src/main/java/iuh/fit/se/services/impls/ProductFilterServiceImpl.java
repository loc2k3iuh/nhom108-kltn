...
public class ProductFilterServiceImpl implements IProductFilterService {
  ...
  public Page<ProductDetailResponse> filterProducts(ProductFilterRequest filterRequest) {
    ...
    Double minAverageRating =
        filterRequest.getMinAverageRating() != null
            ? filterRequest.getMinAverageRating()
    ...
    boolean isAggregateSort =
        "favoriteCount".equalsIgnoreCase(sortBy)
    ...
    if (isAggregateSort) {
      if ("ASC".equalsIgnoreCase(sortDir)) {
        ...
        products =
            productRepository.findProductsWithComplexFilterOrderByAggDesc(
                categoryIds,
                brandIds,
                sizeIds,
                colorIds,
      ...
      products =
          productRepository.findProductsWithComplexFilter(
              categoryIds,
              brandIds,
              sizeIds,
              colorIds,
  ...
  @Override
  public Page<ProductDetailResponse> getProductsByRootCategory(
      Long rootCategoryId, int page, int size, String sortBy, String sortDirection) {
  ...
  @Override
  public Page<ProductDetailResponse> getProductsBySubCategory(
      Long subCategoryId, int page, int size, String sortBy, String sortDirection) {
  ...
  @Override
  public Page<ProductDetailResponse> getProductsByBrand(
      Long brandId, ProductFilterRequest additionalFilters) {
  ...
  @Override
  public Page<ProductDetailResponse> getProductsBySize(
      Long sizeId, ProductFilterRequest additionalFilters) {
  ...
  @Override
  public Page<ProductDetailResponse> getProductsByColor(
      Long colorId, ProductFilterRequest additionalFilters) {
  ...
  @Override
  public Page<ProductDetailResponse> getProductsWithDiscount(
    ...
    log.info(
        "Getting products with discount range: {} - {}", minDiscountPercent, maxDiscountPercent);
    ...
    if (minDiscountPercent != null && maxDiscountPercent != null) {
      products =
          productRepository.findProductsByDiscountRange(
              minDiscountPercent, maxDiscountPercent, pageable);
    } else {
  ...
  @Override
  public Page<ProductDetailResponse> getFavoriteProducts(
  ...
  @Override
  public Page<ProductDetailResponse> getProductsByPriceRange(
      Double minPrice, Double maxPrice, int page, int size, String sortBy, String sortDirection) {
  ...
  public ProductStatisticsResponse getProductStatistics() {
    ...
    Map<String, Long> productCountByBrand =
        brandStats.stream().collect(Collectors.toMap(row -> (String) row[0], row -> (Long) row[1]));

    // Size statistics
    List<Object[]> sizeStats = productRepository.countProductsBySize();
    Map<String, Long> productCountBySize =
        sizeStats.stream().collect(Collectors.toMap(row -> (String) row[0], row -> (Long) row[1]));

    // Color statistics
    List<Object[]> colorStats = productRepository.countProductsByColor();
    Map<String, Long> productCountByColor =
        colorStats.stream().collect(Collectors.toMap(row -> (String) row[0], row -> (Long) row[1]));
    ...
    Double averageStockPerProduct =
        totalActiveProducts > 0 ? totalStock.doubleValue() / totalActiveProducts : 0.0;
  ...
  @Override
  public ProductStatisticsResponse getProductStatisticsByFilter(
  ...
  @Override
  public Page<ProductDetailResponse> getNewProducts(
      int page, int size, String sortBy, String sortDirection) {
    log.info("Getting new products");

    ProductFilterRequest filterRequest =
        ProductFilterRequest.builder()
            .isNew(true)
            .status("ACTIVE")
            .page(page)
            .size(size)
  ...
  @Override
  public Page<ProductDetailResponse> searchProductsAdvanced(
      String keyword, ProductFilterRequest additionalFilters) {
  ...
  private Map<String, Long> calculatePriceRangeStatistics(Double minPrice, Double maxPrice) {
    ...
    if (minPrice != null && maxPrice != null) {
      ...
      priceRangeStats.put(
          String.format("%.0f", minPrice + range)
              + "-"
              + String.format("%.0f", minPrice + 2 * range),
          0L);
      priceRangeStats.put(
          String.format("%.0f", minPrice + 2 * range)
              + "-"
              + String.format("%.0f", minPrice + 3 * range),
          0L);
